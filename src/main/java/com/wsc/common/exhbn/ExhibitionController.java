...
public class ExhibitionController extends BaseController {
	...
	public String open(HttpServletRequest request, Model model) {
		...
		Object result = service.search("getSelectCodeExhbn", params);
		Object result2 = service.search("getSelectCodeSortExhbn", params);
		...
		Object result3 = service.search("searchCodeExhbn", params);
	...
	@RequestMapping(value = "/exhibitionSearchImage.json")
	public String exhibitionSearchImage(HttpServletRequest request, Model model) {
        // 파라메터를 가져온다.
        ParamsMap params = getParams(request);
        
        Object result = service.select("exhibitionSearchImage", params);
	...
	@RequestMapping(value = "/saveExhbnImage.json")
	public String saveExhbnImageOne(HttpServletRequest request, Model model) throws IOException {
        // 파라메터를 가져온다.
        ParamsMap params = getParams(request);
        
        Object result = service.search(params, "saveExhbnImage");

        String atchNo = (String)params.get("atchNo");
        
        String imageData = (String)params.get("atchImg");
        if(imageData == ""){
    		...
    		File outputFile = new File(exhbn_path+"/exhbn/"+atchNo+".png");
    		if(outputFile.exists()) {
    			if(outputFile.delete()) {
    				System.out.println(atchNo+" File delete succeed.");
    				ImageIO.write(image, "png", outputFile);
    			} else {
    				System.out.println(atchNo+" Failed to delete file.");
    			}
    		} else {
    			ImageIO.write(image, "png", outputFile);
    		}
