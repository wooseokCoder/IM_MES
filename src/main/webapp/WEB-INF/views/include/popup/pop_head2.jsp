...
<script>
	...
	    var _top = Math.ceil(( window.screen.height - _height )/2); 
		...
		var orderPop = window.open(context+'/owporder/owporder/owporderstate.do?userId='+userId, 'owporderstate_'+userId,'width='+ _width +',height='+ _height +',left=' + _left + ',top='+ _top);
		
	}
	
	function promoOrderMain(userId) {
		//let hostIndex = location.href.indexOf( location.host ) + location.host.length;
		//let contextPath = location.href.substring( hostIndex, location.href.indexOf('/', hostIndex + 1) );

		window.location.href = context + '/owp';
	}
...
<div class="easyui-layout" data-options="fit:true">
	<div id="topnavSubDiv" data-options="region:'north' ,border:false" class="pop_topnav" >
		...
		<div style="display: flex; align-items: center; padding-right: 20px; width: 130px; justify-content: end;">
			...
			<c:if test="${promoOrderMainYn != 'Y'}">
				<img src="${pageContext.request.contextPath}/resources/images/nav_icons/nav_home.png" onclick="promoOrderMain('${user.userId}');" style="width: 20px; height: 20px; cursor: pointer;" />
				<%-- <a id="btn_main_page" class="btn btn_orderForm" href="javascript:promoOrderMain('${user.userId}');">Main Page</a> --%>
			</c:if>
		</div>
	</div>
