...
consts = {
	...
	url: {
		search: getUrl("/common/board/image/search.json"),
		excel:  getUrl("/common/board/image/download.do"),
		select: getUrl("/common/board/image/select.json"),
		remove: getUrl("/common/board/image/delete.json"),
		save:   getUrl("/common/board/image/save.json"),
		form:   getUrl("/common/board/image/form.do"),
		view:   getUrl("/common/board/image/view.do")
	},
	params: {
		oper: jstatus.INSERT,
		sysId:    null,
...
function doSelectRow(rowIndex, rowData){
	...
	$.ajax({
        ...
        success: function(data){
    		...
    		$(".gallery li a").dblclick(function() {
    		    var address = $(this).children("img");
    		    $("#zoomImg").attr("src",address.attr("src")).attr("alt",address.attr("alt"));
    		    $(this).parent().addClass("on").siblings().removeClass("on");

    			var tmpImg = new Image();
    			tmpImg.src=address.attr("src");

    			var orgWidth = tmpImg.width + 23;
    			var orgHeight = tmpImg.height + 60;

    			$("#zoom-dialog").dialog({title : rowData.bordTitle});
    		    $("#zoom-dialog").dialog({width : orgWidth});
    		    $("#zoom-dialog").dialog({height : orgHeight});
...
function doReplyView(bordNo){
	...
	$.ajax({
        url: getUrl("/common/board/image/replyView.json"),
        type: "POST",
        dataType: "json",
        data: { bordNo: bordNo },
        success: function( data ) {
