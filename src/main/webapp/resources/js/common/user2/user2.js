...
var consts = {
	...
	combo: {
		...
		// incomeDept: new jcombobox({ params: { codeGrup: 'DEPT_CODE' } }), // TSTD_ORG TreeGrid 팝업으로 변경
	...
	init: function () {
		...
		// this.combo.incomeDept.load(); // TSTD_ORG TreeGrid 팝업으로 변경
			...
			if (updatedRow) {
				...
				if (telNo != "" && telNo != undefined) {
					if (!Utils.isStick(telNo)) {
						$.messager.alert(msg.MSG0051, msg.MSG0038, msg.MSG0051);
						return true;
					}
				}
				...
				if (email != "" && email != undefined) {
					if (!Utils.isEmail(email)) {
						$.messager.alert(msg.MSG0051, msg.MSG0039, msg.MSG0051);
						return true;
					}
				}
			} else {
...
// 설비 검색 팝업 (popupbox editor용) - acMachineForm 공통 모듈 사용
function doOpenMcPopup(callback) {
	acMachineForm.open({
		isSap: 1,
		onSelect: function(row) {
			var mainRow = $('#search-grid').datagrid('getSelected');
			if (mainRow) {
				mainRow.ifMcName = row.mcName;
				var rowIndex = $('#search-grid').datagrid('getRowIndex', mainRow);
				if (rowIndex >= 0) {
					var panel = $('#search-grid').datagrid('getPanel');
					panel.find('tr[datagrid-row-index=' + rowIndex + '] td[field=ifMcName] div.datagrid-cell').text(row.mcName);
				}
			}
			if (callback) {
				callback({ value: row.mcCode });
			}
		}
	});
}

// 부서 선택 팝업 (popupbox editor용) - acORGForm 공통 모듈 사용
function doOpenDeptPopup(callback) {
	acORGForm.open({
		//title: 'Department Search',
		onSelect: function(row) {
			// 메인 그리드의 선택된 행에 deptCode 업데이트 (편집 모드 유지)
			var mainRow = $('#search-grid').datagrid('getSelected');
			if (mainRow) {
				mainRow.deptCode = row.orgCode;
			}
			// 콜백 함수 호출 (deptName 에디터에 부서명 설정)
			if (callback) {
				callback({ value: row.orgName });
			}
		}
	});
}
...
function doOpenPopup(callback) {
	...
	if (elm.length == 0) {
		$.messager.alert(msg.MSG0051, msg.MSG0034, msg.MSG0051);
		return false;
	}
...
function doSaveCreate() {
	...
	if ($("#r_userId").textbox("getValue") == "") {
		$.messager.alert(msg.MSG0051, msg.MSG0040, msg.MSG0051);
		return;
	}
	if ($("#r_userName").textbox("getValue") == "") {
		$.messager.alert(msg.MSG0051, msg.MSG0041, msg.MSG0051);
		return;
	}
	if ($("#r_respBm").combobox("getValue") == "") {
		$.messager.alert(msg.MSG0051, msg.MSG0042, msg.MSG0051);
		return;
	}
	...
	/*if($("#r_shipLoc").combobox("getValue") == ""){
		$.messager.alert(msg.MSG0051,msg.MSG0003,msg.MSG0051);
		return;
	}*/
	if ($("#r_email").textbox("getValue") == "") {
		$.messager.alert(msg.MSG0051, msg.MSG0044, msg.MSG0051);
		return;
	}

	//유효성 검사
	if ((!Utils.isNull($("#r_telNo").textbox("getValue"))) && (isNumCheck2($("#r_telNo").textbox("getValue")) == false)) {
		$.messager.alert(msg.MSG0051, msg.MSG0045, msg.MSG0051);
		return;
	}
	...
	if ((!Utils.isNull($("#r_moNo").textbox("getValue"))) && (isNumCheck2($("#r_moNo").textbox("getValue")) == false)) {
		$.messager.alert(msg.MSG0051, msg.MSG0046, msg.MSG0051);
		return;
	}
	...
	if (!Utils.isNull($("#r_faxNo").textbox("getValue")) && (isNumCheck2($("#r_faxNo").textbox("getValue")) == false)) {
		$.messager.alert(msg.MSG0051, msg.MSG0047, msg.MSG0051);
		return;
	}
	...
	if ((!Utils.isNull($("#r_emailList").textbox("getValue"))) && (isNumCheck4($("#r_emailList").textbox("getValue")) == false)) {
		$.messager.alert(msg.MSG0051, msg.MSG0048, msg.MSG0051);
		return;
	}
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.rows.length > 0) {
				...
				$.messager.alert(msg.MSG0051, msg.MSG0062, msg.MSG0051);
			}
