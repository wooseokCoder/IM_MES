var consts = {
	...
	forceFit : true,
	url: {
		search: getUrl("/common/mail/internal/search.json")
	},
	init: function() {
		//기본 컨트롤 컴포넌트 초기화
...
function doToRight(){
	...
	for(var i=0;i<selectGrid.length;i++){
		...
		if(document.getElementById('to_' + selectGrid[i].userId)){
			;
		}else{
			
			var toListTr = "<tr id='to_" + selectGrid[i].userId + "' style='height:25px;'>";
			toListTr += "<td style='padding:5px;'><a herf='javascript:void(0)' onclick='doFlag(this);'>" + selectGrid[i].userId + "</a></td>";
			toListTr += "</tr>"
...
function doCcRight(){
	...
	for(var i=0;i<selectGrid.length;i++){
		//console.log($("#to_list").find("#to_" + selectGrid[i].tgtUserId));
		if(document.getElementById('cc_' + selectGrid[i].userId)){
			;
		}else{
			
			var toListTr = "<tr id='cc_" + selectGrid[i].userId + "' style='height:25px;'>";
			toListTr += "<td style='padding:5px;'><a herf='javascript:void(0)' onclick='doFlag(this);'>" + selectGrid[i].userId + "</a></td>";
			toListTr += "</tr>"
...
function doSelectRow(rowIndex, rowData){
	...
	//console.log($("#hdfIndex").val().indexOf("**"+rowIndex+", "));
	if($("#hdfIndex").val().indexOf("**"+rowIndex+", ") == -1){
		//console.log("in");
		consts.easygrid.clickRowEdit(rowIndex);
		var newRow = oldRow + "**" + rowIndex + ", ";
		$("#hdfIndex").val(newRow);
	}else{
		//console.log("out");
		$(this).datagrid('uncheckRow', rowIndex).datagrid('unselectRow', rowIndex);
		var newRow = oldRow.replace("**" + rowIndex + ", ", "");
		$("#hdfIndex").val(newRow);
	}
}