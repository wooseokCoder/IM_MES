...
//using("../" + jupload_plugin + "/js/jquery.uploadfile.js");
...
var jupload = {
	...
	loaderText: msg.MSG0103,      //파일업로드버튼 명칭
	files     : [],               //보유한 파일데이터
	...
	createGrid: function() {
		...
		using("../" + jupload_plugin + "/js/jquery.uploadfile.js",function(){
		return $("#"+obj.gridKey).datagrid({
			...
			//border:false,
			columns:[[
			    {field:'fileName',width:'65%',halign:'center',align:'left'  ,title:'File name', data_item:'GRD_101',data_popup:'POP_GRD_999',
			    ...
			    {field:'fileSize',width:'15%',halign:'center',align:'center' ,title:'File size', data_item:'GRD_104',data_popup:'POP_GRD_998',
			    	formatter: function(value,row,index) {
			    		return jutils.formatFileSize(value);
			    	}
			    },
			    {field:'fileType',width:'15%',halign:'center',align:'center',title:'File type', data_item:'GRD_102',data_popup:'POP_GRD_997'},
//			    {field:'fileDown',width:'15%',halign:'center',align:'center',title:'Download', data_item:'GRD_103',data_popup:'POP_GRD_996',
//			    	styler: function(value,row,index) {
...
//			    }
			...
			onClickCell: function(index, field, value) {
				if (field == 'fileDown') {
					...
					if (row.exist != true) {
						$.messager.alert(msg.MSG0087, msg.MSG0113, msg.MSG0087);
						return;
					}
					...
					var url = window.location.origin + '/lsdp_data/upload/real' + row.filePath + '/' + row.saveName;
					if(row.saveName.indexOf(".pdf") > 0) {
						window.open("pdf").location.href = url;
					}
					else {
						const element = document.createElement('a');
				        //element.setAttribute('href', '/lsdp_data/upload/real' + row.filePath + '/' + row.saveName);
	...
	createLoader: function() {
	    ...
	    return $("#"+obj.loaderKey).uploadFile({
            ...
            statusBarWidth: '100%',
            dragdropWidth: '100%',
	    	dragDropStr: "<span><b data-popup='POP_TXT_999'>"+msg.MSG0131+"</b></span>",
	    	abortStr :   "Abort",
	    	cancelStr:   "Cancel",
	    	//doneStr  :   "Done",
