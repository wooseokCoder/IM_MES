...
var jupload = {
	...
	createGrid: function() {
		...
		using("../" + jupload_plugin + "/js/jquery.uploadfile3.js",function(){
		return $("#"+obj.gridKey).datagrid({
			...
			//border:false,
			columns:[[
			    {field:'fileName',width:'50%',halign:'center',align:'left'  ,title:'File name', data_item:'GRD_101',data_popup:'POP_GRD_999',
...
var jupload2 = {
		url: {
			//첨부파일 검색 URL
			search: getUrl("/common/file/search2.json"),
			//첨부파일 업로드 URL (임시저장)
			upload: getUrl("/common/file/upload.json"),
			//첨부파일 삭제 URL (임시저장파일 삭제)
			remove: getUrl("/common/file/remove.json"),
		...
		gridKey   : "uploaded-files", //파일그리드 ID (필수)
		loader    : false,            //파일박스객체
		loaderKey : "uploadfile2",     //파일업로드박스 ID
		loaderText: msg.MSG0103,      //파일업로드버튼 명칭
		files     : [],               //보유한 파일데이터
	    	...
	    	$.ajax({
			    ...
			    success: function(data) {
			    	$.each(data.rows, function(i,d) {
			    		//조회상태 처리
			    		jstatus.read(d);
			    		if(jupload2.params.atchNo == d.atchNo) {
			    			loader.createProgress(d);
				    		obj.files.push(d);
			    		}
			    	});
		...
		createGrid: function() {
			...
			using("../" + jupload_plugin + "/js/jquery.uploadfile2.js",function(){
			return $("#"+obj.gridKey).datagrid({
				//title: tit.TITLE0035,
				//영문임시
				title: 'Attached file',
