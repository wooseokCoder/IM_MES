...
var jsystemboard = {
	...
	open: function (status, title) {
		switch (status) {
			...
			case jstatus.INSERT:
				...
				this.formdialog.open(title ? title : tit.TITLE0022);
				break;
			//답변팝업
			case jstatus.REPLY:
				//답변상태 처리
				...
				this.formdialog.open(title ? title : tit.TITLE0016);
				break;
			//수정팝업
			case jstatus.UPDATE:
				//상세폼 닫기
				...
				//수정폼 열기
				this.formdialog.open(title ? title : tit.TITLE0023);
				break;
			//상세팝업
			case jstatus.READ:
				this.consts.setUpdate();
				this.viewdialog.open(title ? title : tit.TITLE0017);
				break;
		}
	},
	//[버튼클릭 핸들러]팝업닫기
	...
	doSave: function () {
		...
		if (cnt > 0) {
			$.messager.alert(msg.MSG0051, msg.MSG0024);
			return;
		}
...
var jsystemaddress = {
	...
	doConfirm: function () {
		...
		if (rows == null ||
			rows.length == 0) {
			$.messager.alert(msg.MSG0051, msg.MSG0017, msg.MSG0051);
			return;
		}
	...
	doAppendUsers: function () {
		...
		if (data.count == 0) {
			$.messager.alert(msg.MSG0051, msg.MSG0001, msg.MSG0051);
			return;
		}
		jlogic.save({
			data: data,
			...
			message: data.count + msg.MSG0069
		});
	},
	...
	doAppendGroup: function () {
		...
		if (data.count == 0) {
			$.messager.alert(msg.MSG0051, msg.MSG0007, msg.MSG0051);
			return;
		}
	...
	doRemoveUsers: function () {
		...
		if (data.count == 0) {
			$.messager.alert(msg.MSG0051, msg.MSG0031, msg.MSG0051);
			return;
		}
		jlogic.save({
			data: data,
			url: jobj.url.remove,
			success: jobj.doUsersResult,
			message: data.count + msg.MSG0032
		});
	},
	...
	doRemoveGroup: function () {
		...
		if (grow == null) {
			$.messager.alert(msg.MSG0051, msg.MSG0008, msg.MSG0051);
			return;
		}
		$.extend(data, grow);
		...
		jlogic.save({
			...
			url: jobj.url.remove,
			success: jobj.doGroupResult,
			message: data.tgtGrupName + msg.MSG0070
		});
	}
};
...
var jboard = {
	...
	initForm2: function (args) {
		...
		if (jstatus.isUpdate(params)) {
			...
			//폼패널 제목
			this.panel.panel('setTitle', this.title + " " + tit.TITLE0023);
		...
		//등록인 경우 버튼활성처리
			...
			//영문임시
			//this.panel.panel('setTitle', this.title + msg.MSG0071);
	...
	initForm: function (args) {
		...
		//등록인 경우 버튼활성처리
			...
			//폼패널 제목
			//this.panel.panel('setTitle', this.title + " " + tit.TITLE0022);
			//영문임시
			this.panel.panel('setTitle', this.title + msg.MSG0071);
...
var jboard2 = {
	...
	initForm2: function (args) {
		...
		if (jstatus.isUpdate(params)) {
			...
			//폼패널 제목
			this.panel.panel('setTitle', this.title + msg.TITLE0024);
