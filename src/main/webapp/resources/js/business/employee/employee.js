...
/*	cellEdit: true,
	...
	forceFit : true,*/
	url: {
		search: getUrl("/business/employee/employee/search.json"),
		excel:  getUrl("/business/employee/employee/download.do"),
		select: getUrl("/business/company/accountmanagement/select.json"),
		remove: getUrl("/business/employee/employee/delete.json"),
		save:   getUrl("/business/employee/employee/save.json")
	},
	init: function() {
		//기본 컨트롤 컴포넌트 초기화
...
function doDblClickRow(idx, row) {
	...
	$("#r_exitYn").combobox("setValue",row.exit_yn_code);
}
