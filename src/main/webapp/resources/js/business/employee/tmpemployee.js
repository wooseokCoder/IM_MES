...
/*	cellEdit: true,
	...
	forceFit : true,*/
	url: {
		search: getUrl("/business/employee/tmpemployee/search.json"),
		excel:  getUrl("/business/employee/tmpemployee/download.do"),
		select: getUrl("/business/company/accountmanagement/select.json"),
		remove: getUrl("/business/employee/tmpemployee/delete.json"),
		save:   getUrl("/business/employee/tmpemployee/save.json")
	},
	init: function() {
		//기본 컨트롤 컴포넌트 초기화
...
$(window).load(function() {
	...
	setTimeout(function (){
		$("#regist-form, #search-toolbar").show();
...
function doSave() {	 
	...
	var busicharge='';
	var i=0;
	$("input[name='checkbusiCharge']:checked").each(function() {
		if(i==0){
			busicharge = $(this).val();
		}else{
			busicharge = busicharge + "/" + $(this).val();
		}		
		console.log(busicharge);
		i++;
	});
	console.log("final: "+busicharge);
	$("input[name='busiCharge").val(busicharge);
	console.log($("input[name='busiCharge").val());
...
function doReload(){
	...
	$("#s_exitYn").combobox('setValue','ALL');
...
/*	 var str = row.busiCharge;
	 var res = str.split("/");
	 console.log(res);
	 

	for (var idx in res) {
		//$("input[name='busiCharge'][value=" + res[idx] + "]").attr("checked", true);
	}*/
