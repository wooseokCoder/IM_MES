...
var consts = {
	...
	url : {
		select: getUrl("/business/item/productsview_new/select.json"),
		search : getUrl("/business/item/productsview_new/search.json"),
		excel : getUrl("/business/item/productsview_new/download.do"),
		remove : getUrl("/business/item/productsview_new/delete.json"),
		save : getUrl("/business/item/productsview_new/save.json"),
	},
	init : function() {
		// 콤보로딩
...
var consts_item = {
	...
	easygrid : false, // 기본컨트롤
	url : {
		search : getUrl("/business/item/productsview_new/searchitem.json")
	},
	init : function() {
		...
		this.easygrid.init({
			...
			onDblClickRow : function(index, row) {
				...
				$('#searchitemMaterial').textbox('setValue', row.itemMaterial);
...
function doSave1() {
	...
	var ProdType='';
	var i=0;
	$("input[name='checkProdType']:checked").each(function() {
		if(i==0){
			ProdType = $(this).val();
		}else{
			ProdType = ProdType + "/" + $(this).val();
		}		
		console.log(ProdType);
		i++;
	});
	$("input[name='prodType").val(ProdType);
...
function doSave(){
	...
	console.log("final: "+ProdType);
	$("input[name='prodType").val(ProdType);
	console.log($("input[name='prodType").val());
	
  $.ajax({
      url: getUrl('/business/item/productsview_new/saveItem.json'),
      dataType: 'json',
      async: false,
      type: 'post',
      data: {
    	 ...
    	 ,itemMaterial:$("#r_itemMaterial").val()    	 
    	 ...
    	 ,modelNo:$("#r_modelNo").combobox("getValue")
    	 ,modelName:$("#r_modelName").val()
    	 ,itemBar:$("#r_itemBar").val()
    	 ,itemBarMax:$("#r_itemBarMax").val()
    	 ...
    	 ,prodType:$("#r_prodType").val()
    	 ,moldNo:$("#r_moldNo").val()
    	 ,factory:$("#r_factory").combobox("getValue")
...
function doExcel() {
	...
	$.ajax({
	      url: getUrl('/business/item/productsview_new/searchCount.json'),
	      dataType: 'json',
	      async: false,
	      type: 'post',
	      data: {searchitemName:$("#searchitemName").textbox("getValue")
	    	  	,searchitemMaterial:$("#searchitemMaterial").textbox("getValue")
...
function SettingTab2Default(){
	 ...
	 $("#r_itemSpec2").val("");
	 $("#r_itemMaterial2").val("");
	 /*$("#r_factory").combobox('setValue','');*/	 
	 $("#r_moldNo").val("");
	 
	 $("input:checkbox[id='01']").prop("checked", false); 
	 $("input:checkbox[id='02']").prop("checked", false); 
	 $("input:checkbox[id='03']").prop("checked", false);
	 $("input:checkbox[id='04']").prop("checked", false); 
	 $("input:checkbox[id='05']").prop("checked", false); 
	 $("input:checkbox[id='06']").prop("checked", false);
	 $("input:checkbox[id='07']").prop("checked", false); 
	 $("input:checkbox[id='08']").prop("checked", false); 
}
function doReload(){
		$("#searchitemName").textbox('setValue','');
		$("#searchitemMaterial").textbox('setValue','');
...
//	 consts.easygrid.update(row);
	 
	 $("#r_itemCode2").val(row.itemCode);
	 $("#r_itemName2").val(row.itemName);
	 $("#r_itemNameAlt2").val(row.itemNameAlt);
	 $("#r_itemSpec2").val(row.itemSpec);
	 $("#r_itemMaterial2").val(row.itemMaterial);
	 $("#r_moldNo").val(row.moldNo);
	 $("#r_factory").combobox('setValue',row.factory);
	 
	if(row.prodType){
		 var str = row.prodType;
		 var res = str.split("/");		 
		 ...
		 for(var i=0;i<res.length;i++){
			 ...
			 switch(res[i]){
			 case '01':
				 $("input:checkbox[id='01']").prop("checked", true); 
				 break;
			 case '02':
				 $("input:checkbox[id='02']").prop("checked", true); 
				 break;
			 case '03':
				 $("input:checkbox[id='03']").prop("checked", true); 
				 break;
			 case '04':
				 $("input:checkbox[id='04']").prop("checked", true); 
				 break;
			 case '05':
				 $("input:checkbox[id='05']").prop("checked", true); 
				 break;
			 case '06':
				 $("input:checkbox[id='06']").prop("checked", true); 
				 break;
			 case '07':
				 $("input:checkbox[id='07']").prop("checked", true); 
				 break;
			 case '08':
				 $("input:checkbox[id='08']").prop("checked", true); 
				 break;
			default:
				break;
			 }
...
function selectItemType(rec){
	...
	if(itemType !=""){
			$.ajax({
	        url: getUrl('/business/item/productsview_new/selectItemTypeCode2.json'),
	        dataType: 'json',
	        async: false,
	        type: 'post',
...
//모델번호 가져오기
function getModelNo(rec){
	$.ajax({
	    url: getUrl('/business/item/productsview_new/getModelNo.json'),
	    dataType: 'json',
	    async: false,
	    type: 'post',
	    data: {},
	    success: function(data){
	    	$("#r_modelNo").combobox('setValue', '');
	    	var combodata = $('#r_modelNo').combobox('getData');
	    	if(data.rows.length != 0){
	    		for(var i=0; i < data.rows.length; i++){
	    			combodata.push({value: data.rows[i].modelNo,text:data.rows[i].modelNo});
				}
				$('#r_modelNo').combobox('loadData', combodata);
	    	}else{
	    		$("#r_modelNo").combobox('setValue', '');
	    	}
	    },
...
function getModelName(rec){
	var modelNo = $("#r_modelNo").combobox('getValue');
	$.ajax({
	    url: getUrl('/business/item/productsview_new/getModelNo.json'),
	    dataType: 'json',
	    async: false,
	    ...
	    success: function(data){
	    	if(data.rows.length != 0){
				$('#r_modelName').val(data.rows[0].modelName);
	    	}else{
	    		$("#r_modelName").val('');
	    	}
	    },
