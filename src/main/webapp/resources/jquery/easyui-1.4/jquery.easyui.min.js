...
t.attr("data-item",_8c.data_item);//20161206 김원국
t.attr("data-popup",_8c.data_popup);//20161206 김원국
if(_8c.plain){
...
var t=$(_9d);
return $.extend({},$.parser.parseOptions(_9d,["id","iconCls","iconAlign","group","size","data_item","data_popup",{plain:"boolean",toggle:"boolean",selected:"boolean"}]),{disabled:(t.attr("disabled")?true:undefined),text:$.trim(t.html()),iconCls:(t.attr("icon")||t.attr("iconCls"))});
};
...
$.fn.pagination.defaults={total:1,pageSize:10,pageNumber:1,pageList:[50,100,500,1000],loading:false,buttons:null,showPageList:true,showRefresh:true,links:10,layout:["list","sep","first","prev","sep","manual","sep","next","last","sep","refresh"],onSelectPage:function(_c4,_c5){
...
//2016/09/29 김영진 체크 좌측메뉴 마우스 오버
//console.log(_d8);
if(!_d8.hasClass("noAuth")){
...
//2016/09/30 김영진 체크 좌측  트리메뉴 클릭 이벤트
var tt=$(e.target);
var _da=tt.closest("div.tree-node");
//console.log(_da);
if(_da.hasClass("noAuth")){
return;
}
if(!_da.length){
return;
...
//2016/09/30 김영진 체크 좌측 트리메뉴 마우스 오륵쪽 클릭
var _dc=$(e.target).closest("div.tree-node");
//console.log(_dc);
if(_dc.hasClass("noAuth")){
return;
}
if(!_dc.length){
return;
...
var item=_1b8[i];
// //2016/09/29 김영진 체크 트리메뉴 아이콘 생성 및 권한설정
if(item.state!="open"&&item.state!="closed"){
item.state="open";
}
item.domId="_easyui_tree_"+_1b2++;
cc.push("<li>");
//20180810 박민혁 서브메뉴 3단메뉴
cc.push("<div id=\""+item.domId+"\" "+      (item.attributes.submenuYn == 'Y' && item.attributes.menuLevel == 2?"onclick=\"submenuOpen('"+item.attributes.menuKey+"','"+item.domId+"')\"":"")+		(item.attributes.submenuYn == 'Y' && item.attributes.menuLevel == 3?"onclick=\"submenuOpen2('"+item.attributes.menuKey+"','"+item.domId+"')\"":"")+			" class=\"tree-node "+(item.attributes.menuLevel == 3?"tree-level3":"")+" "+(item.attributes.menuLevel == 4?"tree-level4":"")+" "+(item.attributes.menuLevel == 3 || item.attributes.menuLevel == 4?item.attributes.parentKey:"")+" "+(item.enableYn == "N"?"noAuth":"")+" "+(item.progAuth == "1"?"":"noAuth")+"\">");
...
/*if(item.iconCls){
cc.push("<i class=\"tree-icon fa "+ item.iconCls +"\"></i>");
}else{
cc.push("<span class=\"tree-icon tree-file\"></span>");
}*/
//
cc.push("<span class=\"tree-icon fa "+(item.iconCls?item.iconCls:"")+"\"></span>");
_1b9=true;
}
}
if(opts.checkbox){
...
//20180820 박민혁 3단메뉴 화살표 추가
if(item.attributes.submenuYn == 'Y' && (item.attributes.menuLevel == 2 || item.attributes.menuLevel == 3)){
cc.push("<span class=\"tree-icon fa fa-angle-right tree-submenu-icon-angle-right\"></span>");
}
cc.push("</div>");
if(item.children&&item.children.length){
...
//alert(_201.width());
_201._size(opts);
//BBUG.KWK _201.width() > > 100%
_202.add(_203)._outerWidth('100%');
...
//2016/10/05 김영진 체크 --region layout 이동 이벤트
...
var onClickEvent = "";//2016 12 14 김원국 hot Menu 버그 수정
if(_20e.id == 'left-hotmenu'){
	onClickEvent="id=\"panel-tool-collapse\" onClick=\"javascript:MenuMove()\"";
}
if(opts.collapsible){
$("<a class=\"panel-tool-collapse\" href=\"javascript:void(0)\" "+onClickEvent+" ></a>").appendTo(tool).bind("click",function(){
...
function _28e(_28f){
var opts=$.extend({},$.fn.window.defaults,{collapsible:false,minimizable:false,maximizable:false,shadow:false,draggable:false,resizable:false,closed:true,style:{left:"",top:"",right:0,zIndex:$.fn.window.defaults.zIndex++,bottom:0, position: "fixed"},onBeforeOpen:function(){
...
//2016/10/06 김영진 --collapsed:(pp[0].id=="menu-region"?true:false) 좌측 메뉴 초기상태를 접힘으로 설정.
var _388=$.extend({},$.fn.layout.paneldefaults,{width:(pp.length?parseInt(pp[0].style.width)||pp.outerWidth():"auto"),height:(pp.length?parseInt(pp[0].style.height)||pp.outerHeight():"auto"),collapsed:(pp[0].id=="menu-region"?true:false),doSize:false,collapsible:true,cls:("layout-panel layout-panel-"+dir),bodyCls:"layout-body",onOpen:function(){
...
$(_50a).find(".calendar-title span").html(opts.year +" - " + opts.months[opts.month-1]);
...
//$.fn.calendar.defaults={width:180,height:180,fit:false,border:true,firstDay:0,weeks:["일", "월", "화", "수", "목", "금", "토"],months:["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"],year:new Date().getFullYear(),month:new Date().getMonth()+1,current:(function(){
var d=new Date();
...
if(datagrid_id != '#') {
	//var panel = $(datagrid_id).datagrid('getPanel').attr('tabindex',0).focus();
...
var th=$(this);
var col=$.extend({},$.parser.parseOptions(this,["field","align","halign","order","width","data_item","data_popup",{sortable:"boolean",checkbox:"boolean",resizable:"boolean",fixed:"boolean"},{rowspan:"number",colspan:"number"}]),{title:(th.html()||undefined),hidden:(th.attr("hidden")?true:undefined),formatter:(th.attr("formatter")?eval(th.attr("formatter")):undefined),styler:(th.attr("styler")?eval(th.attr("styler")):undefined),sorter:(th.attr("sorter")?eval(th.attr("sorter")):undefined)});
...
if(col.data_item){td.attr("data-item",col.data_item);}//20161124 김원국
if(col.data_popup){td.attr("data-popup",col.data_popup);}//20161206김원국
if(col.checkbox){
...
//2016/09/20 김영진 수정 --mouseout시 삭제된 클래스 추가
//opts.finder.getTr(_5ba,_5c6).removeClass("datagrid-row-over");
var _631 = $.data(_5ba, "datagrid");
var opts = _631.options;
opts.finder.getTr(_5ba, _631.highlightIndex).removeClass("datagrid-row-over");
opts.finder.getTr(_5ba, _631.highlightIndex).addClass(function() {return (_631.highlightIndex % 2 && !opts.finder.getTr(_5ba, _631.highlightIndex).hasClass("datagrid-row-selected-check") ? "datagrid-row-alt" : "");});
opts.finder.getTr(_5ba, _631.highlightIndex).addClass(function() {return (opts.finder.getTr(_5ba, _631.highlightIndex).hasClass("datagrid-row-selected-check") ? "datagrid-row-selected" : "");});
}).bind("click",function(e){
var tt=$(e.target);
 ...
 * BBUG.kwk
 * date : 20150715
 * opts.finder.getTr(_62f,_630.highlightIndex).removeClass(function(){ return (opts.singleSelect?"datagrid-row-selected-check":"");});의 문법에서
 * _630.highlightIndex쪽 추가
 */
//2016/09/20 김영진 수정 --mouseover시 이전 행 클래스 조작 주석처리
//alert("jquery.easyui.min.js : datagrid-row-over - 222");

var rows=opts.finder.getRows(_62f);
//if(_631.highlightIndex >= 0 && _631.highlightIndex < rows.length){
	...
	//opts.finder.getTr(_62f,_630).removeClass(function(){ return ((_630==(rows.length-1)|| _630 == 0 )&& opts.finder.getTr(_62f,_630).hasClass("datagrid-row-selected-check")?"":"datagrid-row-selected");});
	...
	opts.finder.getTr(_62f,_630.highlightIndex).removeClass(function(){ return (opts.singleSelect?"datagrid-row-selected-check":"");});
...
//2016/09/20 김영진 수정 --체크박스가 선택된 상태에선 체크박스 클릭으로만 선택 해제 가능하게
if(!opts.finder.getTr(_633,_634).hasClass("datagrid-row-selected-check")||opts.finder.getTr(_633,_634).hasClass("datagrid-row-checked")){
...
opts.finder.getTr(_633,_634).removeClass("datagrid-row-alt");
...
if(!opts.finder.getTr(_63b,_63c).hasClass("datagrid-row-checked")){  //2016/09/20 김영진 수정 --체크박스가 선택되어있으면 행선택 유지
...
/* BBUG - kwk : 추가 -STR
 * date : 20150715 
 * datagrid-row-selected-check일떄 datagrid-row-selected-check 및 datagrid-row-selected 와 전 선택된 datagrid-row-selected-check 의 홀수 로우 datagrid-row-alt로 변
 */
for(var _64e=0; _64e < rows.length; _64e++){
	if(_64e%2 == 1){
		//alert(_64e+":"+opts.finder.getTr(_648,_64e).hasClass("datagrid-row-selected"));
		opts.finder.getTr(_648,_64e).addClass(function(){ return (opts.finder.getTr(_648,_64e).hasClass("datagrid-row-selected-check")?"datagrid-row-alt":"");});
	}
}
opts.finder.getTr(_648,"","allbody").removeClass("datagrid-row-selected");
opts.finder.getTr(_648,"","allbody").removeClass("datagrid-row-selected-check");
/* BBUG - kwk : 추가 -END*/
...
//BBUG.KWK 20150723 추가
opts.finder.getTr(_648,_64a.highlightIndex).removeClass("datagrid-row-alt");
...
//체크 클릭 : 미선택 -> 선택 시
//alert("selectOnCheck - 000");
//2016/09/20 김영진 수정 --행클릭 후 체크박스 선택 시 datagrid-row-selected class가 사라져버려서 체크박스 선택 시 selected와 selected-check class추가 
if(!_650&&opts.selectOnCheck){
_632(_64e,_64f,true);
}
var tr=opts.finder.getTr(_64e,_64f).addClass("datagrid-row-checked");
opts.finder.getTr(_64e,_64f).addClass("datagrid-row-selected");
opts.finder.getTr(_64e,_64f).addClass("datagrid-row-selected-check");
...
//체크 해제
//alert("selectOnCheck - 111");
//2016/09/20 김영진 수정 --체크박스 해제 시 행선택 해제
if(!_655&&opts.selectOnCheck){
_63a(_653,_654,true);
}
var tr=opts.finder.getTr(_653,_654).removeClass("datagrid-row-checked");
opts.finder.getTr(_653,_654).removeClass("datagrid-row-selected");
opts.finder.getTr(_653,_654).removeClass("datagrid-row-selected-check");
var ck=tr.find("div.datagrid-cell-check input[type=checkbox]");
...
$("head").append("<style id=\"datagrid-group-style\">"+".datagrid-group{height:25px;overflow:auto;font-weight:bold;border-bottom:1px solid #ccc;}"+"</style>");
}
};
}});
