-- ============================================================
-- Bulletin.xml -> 프로시저 전환 스크립트
-- 생성일: 2026-01-14
-- 파일: sp_bulletin_procedures.sql
...
-- [sp_bulletin_search] 게시글 목록 조회 (페이징 및 동적 검색)
DROP PROCEDURE IF EXISTS sp_bulletin_search//
CREATE PROCEDURE sp_bulletin_search(
        ...
        SELECT X.* FROM (
            SELECT Z1.*, @rownum := @rownum + 1 AS RNUM FROM (
                ...
                WHERE A.SYS_ID = '", p_sys_id, "'
                  ...
                  ", IF(p_use_flag IS NOT NULL AND p_use_flag != '', CONCAT(" AND A.USE_FLAG = '", p_use_flag, "'"), ""), "
                  ", IF(p_regi_id IS NOT NULL AND p_regi_id != '', CONCAT(" AND A.REGI_ID = '", p_regi_id, "'"), ""), "
                  ", IF(p_bord_title IS NOT NULL AND p_bord_title != '', CONCAT(" AND A.BORD_TITLE LIKE '%", p_bord_title, "%'"), ""), "
                  ", IF(p_bord_text IS NOT NULL AND p_bord_text != '', CONCAT(" AND A.BORD_TEXT LIKE '%", p_bord_text, "%'"), ""), "
                  ", IF(p_bord_type IS NOT NULL AND p_bord_type != '', CONCAT(" AND A.BORD_TYPE = '", p_bord_type, "'"), ""), "
                  ", IF(p_bord_bgn IS NOT NULL AND p_bord_bgn != '', CONCAT(" AND A.BORD_BGN = '", p_bord_bgn, "'"), ""), "
                  ", IF(p_bord_end IS NOT NULL AND p_bord_end != '', CONCAT(" AND A.BORD_END = '", p_bord_end, "'"), ""), "
                  ", IF(p_open_type IS NOT NULL AND p_open_type != '', CONCAT(" AND A.OPEN_TYPE = '", p_open_type, "'"), ""), "
                  ", IF(p_search_key = 'S01' AND p_search_text IS NOT NULL AND p_search_text != '', CONCAT(" AND A.BORD_TITLE LIKE '%", p_search_text, "%'"), ""), "
                  ", IF(p_search_key = 'S02' AND p_search_text IS NOT NULL AND p_search_text != '', CONCAT(" AND A.BORD_TEXT LIKE '%", p_search_text, "%'"), ""), "
                  ", IF(p_search_key = 'S03' AND p_search_text IS NOT NULL AND p_search_text != '', CONCAT(" AND EXISTS (SELECT 1 FROM SYS_USER WHERE SYS_ID = A.SYS_ID AND USER_ID = A.REGI_ID AND USER_NAME LIKE '%", p_search_text, "%')"), ""), "
                  ", IF(p_bord_pno IS NOT NULL AND p_bord_pno != '', CONCAT(" AND A.BORD_PNO = '", p_bord_pno, "'"), " AND A.BORD_PNO IS NULL"), "
                  ", IF(p_bord_grup = 'B04', " AND ((A.USE_FLAG = 'Y') OR (A.USE_FLAG = 'N' AND (SELECT COUNT(*) FROM SYS_BORD WHERE SYS_ID = A.SYS_ID AND BORD_PNO = A.BORD_NO AND BORD_GRUP = A.BORD_GRUP AND USE_FLAG = 'Y') > 0))", ""), "
                  ", IF(p_today_yn = 'Y', " AND DATE_FORMAT(NOW(),'%Y-%m-%d') BETWEEN A.BORD_BGN AND A.BORD_END", ""), "
                ORDER BY ", @sort_order, "
            ) Z1, (SELECT @rownum:=0) Z2
        ) X
        ", IF(p_end IS NOT NULL, CONCAT(" WHERE RNUM < ", p_end), ""), "
    ) X
    ", IF(p_start IS NOT NULL, CONCAT(" WHERE RNUM >= ", p_start), ""));
...
-- [sp_bulletin_search_count] 게시글 카운트
DROP PROCEDURE IF EXISTS sp_bulletin_search_count//
CREATE PROCEDURE sp_bulletin_search_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_regi_id VARCHAR(50),
...
-- [sp_bulletin_select] 게시글 상세 조회
DROP PROCEDURE IF EXISTS sp_bulletin_select//
CREATE PROCEDURE sp_bulletin_select(
    IN p_sys_id VARCHAR(20),
...
-- [sp_bulletin_get_next_bord_no] 다음 게시글 번호 생성
DROP PROCEDURE IF EXISTS sp_bulletin_get_next_bord_no//
CREATE PROCEDURE sp_bulletin_get_next_bord_no(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
...
-- [sp_bulletin_insert] 게시글 등록
DROP PROCEDURE IF EXISTS sp_bulletin_insert//
CREATE PROCEDURE sp_bulletin_insert(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_title VARCHAR(200),
    IN p_bord_type VARCHAR(20),
...
BEGIN
    ...
    -- 다음 게시글 번호 생성
    CALL sp_bulletin_get_next_bord_no(p_sys_id, p_bord_grup, v_bord_no);
...
-- [sp_bulletin_update] 게시글 수정
DROP PROCEDURE IF EXISTS sp_bulletin_update//
CREATE PROCEDURE sp_bulletin_update(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_title VARCHAR(200),
...
-- [sp_bulletin_delete] 게시글 삭제
DROP PROCEDURE IF EXISTS sp_bulletin_delete//
CREATE PROCEDURE sp_bulletin_delete(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_bulletin_update_read_cnt] 조회수 증가
DROP PROCEDURE IF EXISTS sp_bulletin_update_read_cnt//
CREATE PROCEDURE sp_bulletin_update_read_cnt(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_bulletin_update_disable] 게시글 비활성화
DROP PROCEDURE IF EXISTS sp_bulletin_update_disable//
CREATE PROCEDURE sp_bulletin_update_disable(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_gs_user_id VARCHAR(50)
...
-- [sp_bulletin_search_target] 대상자 목록 조회
DROP PROCEDURE IF EXISTS sp_bulletin_search_target//
CREATE PROCEDURE sp_bulletin_search_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_bord_no VARCHAR(20),
        ...
        SELECT X.* FROM (
            SELECT Z1.*, @rownum := @rownum + 1 AS RNUM FROM (
                ...
                WHERE A.SYS_ID = '", p_sys_id, "'
                  AND A.BORD_GRUP = '", p_bord_grup, "'
                  ", IF(p_use_flag IS NOT NULL AND p_use_flag != '', CONCAT(" AND A.USE_FLAG = '", p_use_flag, "'"), ""), "
                  ", IF(p_bord_no IS NOT NULL AND p_bord_no != '', CONCAT(" AND A.BORD_NO = '", p_bord_no, "'"), ""), "
                  ", IF(p_vndr_code IS NOT NULL AND p_vndr_code != '', CONCAT(" AND A.VNDR_CODE = '", p_vndr_code, "'"), ""), "
                  ", IF(p_vndr_name IS NOT NULL AND p_vndr_name != '', CONCAT(" AND A.VNDR_NAME = '", p_vndr_name, "'"), ""), "
                  ", IF(p_save_idx IS NOT NULL, CONCAT(" AND A.SAVE_IDX = ", p_save_idx), ""), "
                  ", IF(p_page_type = 'R', CONCAT(" AND A.TGT_USER_ID = '", IFNULL(p_gs_user_id,''), "'"), ""), "
                  ", IF(p_page_type = 'S', CONCAT(" AND A.REGI_ID = '", IFNULL(p_gs_user_id,''), "'"), ""), "
                ORDER BY ", @sort_order, "
            ) Z1, (SELECT @rownum:=0) Z2
        ) X
        ", IF(p_end IS NOT NULL, CONCAT(" WHERE RNUM < ", p_end), ""), "
...
-- [sp_bulletin_search_target_count] 대상자 목록 카운트
DROP PROCEDURE IF EXISTS sp_bulletin_search_target_count//
CREATE PROCEDURE sp_bulletin_search_target_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_bord_no VARCHAR(20),
...
-- [sp_bulletin_select_target] 대상자 단건 조회
DROP PROCEDURE IF EXISTS sp_bulletin_select_target//
CREATE PROCEDURE sp_bulletin_select_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
...
-- [sp_bulletin_insert_target] 대상자 등록
DROP PROCEDURE IF EXISTS sp_bulletin_insert_target//
CREATE PROCEDURE sp_bulletin_insert_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_targets TEXT,
...
-- [sp_bulletin_update_target] 대상자 정보 수정
DROP PROCEDURE IF EXISTS sp_bulletin_update_target//
CREATE PROCEDURE sp_bulletin_update_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50),
...
-- [sp_bulletin_update_target_disable] 대상자 비활성화
DROP PROCEDURE IF EXISTS sp_bulletin_update_target_disable//
CREATE PROCEDURE sp_bulletin_update_target_disable(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50),
...
-- [sp_bulletin_delete_target_all] 대상자 일괄 삭제
DROP PROCEDURE IF EXISTS sp_bulletin_delete_target_all//
CREATE PROCEDURE sp_bulletin_delete_target_all(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_bulletin_update_target_read] 대상자 읽음 처리
DROP PROCEDURE IF EXISTS sp_bulletin_update_target_read//
CREATE PROCEDURE sp_bulletin_update_target_read(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50)
