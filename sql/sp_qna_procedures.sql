-- ============================================================
-- Qna.xml (QnA 게시판) -> 프로시저 전환 스크립트
-- 생성일: 2026-01-14
-- 파일: sp_qna_procedures.sql
-- ============================================================

DELIMITER //

-- ============================================================
-- 1. QnA 게시글 관리 (SYS_BORD)
-- ============================================================

-- [sp_qna_search] QnA 목록 조회 (페이징 및 동적 검색)
DROP PROCEDURE IF EXISTS sp_qna_search//
CREATE PROCEDURE sp_qna_search(
...
-- [sp_qna_search_count] QnA 카운트
DROP PROCEDURE IF EXISTS sp_qna_search_count//
CREATE PROCEDURE sp_qna_search_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_regi_id VARCHAR(50),
...
-- [sp_qna_select] QnA 상세 조회
DROP PROCEDURE IF EXISTS sp_qna_select//
CREATE PROCEDURE sp_qna_select(
    IN p_sys_id VARCHAR(20),
...
-- [sp_qna_get_next_bord_no] 다음 게시글 번호 조회
DROP PROCEDURE IF EXISTS sp_qna_get_next_bord_no//
CREATE PROCEDURE sp_qna_get_next_bord_no(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
...
-- [sp_qna_insert] QnA 등록
DROP PROCEDURE IF EXISTS sp_qna_insert//
CREATE PROCEDURE sp_qna_insert(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_title VARCHAR(200),
    IN p_bord_type VARCHAR(20),
...
BEGIN
    ...
    INSERT INTO SYS_BORD (
        ...
        REGI_ID, REGI_DATE, CHNG_ID, CHNG_DATE, BORD_TYPE, BORD_BGN, BORD_END, BORD_PNO, OPEN_TYPE, BORD_TEXT
    ) VALUES (
        p_sys_id, v_bord_no, p_bord_grup, p_bord_title, 0, 999999999, 'Y',
...
-- [sp_qna_update] QnA 수정
DROP PROCEDURE IF EXISTS sp_qna_update//
CREATE PROCEDURE sp_qna_update(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_title VARCHAR(200),
...
BEGIN
    ...
    SET CHNG_ID = p_gs_user_id,
        ...
        BORD_TEXT = IF(p_bord_text IS NOT NULL AND p_bord_text != '', p_bord_text, BORD_TEXT),
...
-- [sp_qna_delete] QnA 물리 삭제
DROP PROCEDURE IF EXISTS sp_qna_delete//
CREATE PROCEDURE sp_qna_delete(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_qna_update_read_cnt] 조회수 증가
DROP PROCEDURE IF EXISTS sp_qna_update_read_cnt//
CREATE PROCEDURE sp_qna_update_read_cnt(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_qna_update_disable] QnA 비활성화 처리
DROP PROCEDURE IF EXISTS sp_qna_update_disable//
CREATE PROCEDURE sp_qna_update_disable(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_gs_user_id VARCHAR(50)
...
-- ============================================================
-- 2. QnA 대상자 관리 (SYS_BORD_TGT)
-- ============================================================

-- [sp_qna_search_target] QnA 대상자 목록 조회
DROP PROCEDURE IF EXISTS sp_qna_search_target//
CREATE PROCEDURE sp_qna_search_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_bord_no VARCHAR(20),
...
-- [sp_qna_search_target_count] QnA 대상자 목록 카운트
DROP PROCEDURE IF EXISTS sp_qna_search_target_count//
CREATE PROCEDURE sp_qna_search_target_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_bord_no VARCHAR(20),
...
-- [sp_qna_select_target] QnA 대상자 단건 조회
DROP PROCEDURE IF EXISTS sp_qna_select_target//
CREATE PROCEDURE sp_qna_select_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
...
-- [sp_qna_insert_target] QnA 대상자 등록
DROP PROCEDURE IF EXISTS sp_qna_insert_target//
CREATE PROCEDURE sp_qna_insert_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_targets TEXT,
...
-- [sp_qna_update_target] QnA 대상자 정보 수정
DROP PROCEDURE IF EXISTS sp_qna_update_target//
CREATE PROCEDURE sp_qna_update_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50),
...
-- [sp_qna_update_target_disable] QnA 대상자 비활성화 처리
DROP PROCEDURE IF EXISTS sp_qna_update_target_disable//
CREATE PROCEDURE sp_qna_update_target_disable(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50),
...
-- [sp_qna_delete_target_all] QnA 대상자 일괄 삭제
DROP PROCEDURE IF EXISTS sp_qna_delete_target_all//
CREATE PROCEDURE sp_qna_delete_target_all(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_qna_update_target_read] QnA 대상자 읽음 처리
DROP PROCEDURE IF EXISTS sp_qna_update_target_read//
CREATE PROCEDURE sp_qna_update_target_read(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50)
...
-- [sp_qna_select_reply_list] 답변 목록 조회
DROP PROCEDURE IF EXISTS sp_qna_select_reply_list//
CREATE PROCEDURE sp_qna_select_reply_list(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_pno VARCHAR(20)
)
BEGIN
    ...
    WHERE A.SYS_ID = p_sys_id
      AND A.BORD_GRUP = p_bord_grup
      AND A.BORD_PNO = p_bord_pno
      AND A.USE_FLAG = 'Y'
    ORDER BY A.REGI_DATE ASC;
END//

-- [sp_qna_select_reply_view] 답변 상세 조회
DROP PROCEDURE IF EXISTS sp_qna_select_reply_view//
CREATE PROCEDURE sp_qna_select_reply_view(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_no VARCHAR(20)
)
