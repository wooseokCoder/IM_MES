-- ============================================================
-- SvcCode.xml 쿼리 -> 프로시저 전환 스크립트
-- 생성일: 2026-01-13
-- 수정일: 2026-01-14 (누락된 프로시저 추가)
...
-- 1. 서비스 코드 목록 조회 (페이징)
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search//

CREATE PROCEDURE sp_svccode_search(
            ...
            FROM (
                SELECT
                    ...
                    CASE WHEN CODE_GRUP = '0' THEN CODE_NAME
                         ELSE (SELECT DISTINCT B.CODE_NAME FROM SYS_CODE B WHERE B.SYS_ID = 'WSC' AND B.CODE_CD = CODE_GRUP GROUP BY B.CODE_CD)
                    END AS codeGrup2,
                    CODE_CD AS codeCd,
                ...
                ORDER BY
                    CASE WHEN p_sort_str IS NOT NULL AND p_sort_str != '' THEN NULL ELSE SORT_SEQ END
            ) Z1
        ) X
...
-- 2. 서비스 코드 목록 카운트
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_count//

CREATE PROCEDURE sp_svccode_search_count(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_desc VARCHAR(200),
    IN p_use_flag VARCHAR(10),
...
-- 3. 서비스 코드 단건 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_select//

CREATE PROCEDURE sp_svccode_select(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_gs_lang VARCHAR(10)
)
...
-- 4. 서비스 코드 등록
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_insert//

CREATE PROCEDURE sp_svccode_insert(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
...
BEGIN
    DECLARE v_code_grup VARCHAR(50);

    -- codeGrup이 있으면 SERVICE_M, 없으면 '0'
    SET v_code_grup = CASE WHEN p_code_grup IS NOT NULL AND p_code_grup != '' THEN 'SERVICE_M' ELSE '0' END;

    INSERT INTO SYS_CODE (
        SYS_ID, CODE_CD, CODE_GRUP, CODE_NAME, CODE_NAME_EN,
        CODE_DESC, SORT_SEQ, USE_FLAG,
        EXT_CHR01, EXT_CHR02, EXT_CHR03, EXT_CHR04, EXT_CHR05,
        EXT_CHR06, EXT_CHR07, EXT_CHR08, EXT_CHR09, EXT_CHR10,
        EXT_NUM01, EXT_NUM02, EXT_NUM03, EXT_NUM04, EXT_NUM05,
        EXT_TEXT, REGI_ID, REGI_DATE, CHNG_ID, CHNG_DATE
    ) VALUES (
        p_sys_id, p_code_cd, v_code_grup, p_code_cd, p_code_cd,
        p_code_desc, p_sort_seq, IFNULL(p_use_flag, 'Y'),
        p_ext_chr01, p_ext_chr02, p_ext_chr03, p_ext_chr04, p_ext_chr05,
        p_ext_chr06, p_ext_chr07, p_ext_chr08, p_ext_chr09, p_ext_chr10,
        p_ext_num01, p_ext_num02, p_ext_num03, p_ext_num04, p_ext_num05,
        p_ext_text, p_gs_user_id, NOW(), p_gs_user_id, NOW()
    );
END//
...
-- 5. 서비스 코드 수정
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_update//

CREATE PROCEDURE sp_svccode_update(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_desc VARCHAR(500),
    IN p_sort_seq VARCHAR(20),
...
BEGIN
    ...
    SET CHNG_ID = p_gs_user_id,
        CHNG_DATE = NOW(),
        CODE_NAME = p_code_cd,
        CODE_NAME_EN = p_code_cd,
        CODE_DESC = IFNULL(p_code_desc, ''),
...
-- 6. 서비스 코드 삭제
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_delete//

CREATE PROCEDURE sp_svccode_delete(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50)
)
BEGIN
...
-- 7. 서비스 코드 전체 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_all//

CREATE PROCEDURE sp_svccode_search_all(
    IN p_sys_id VARCHAR(20),
    IN p_gs_lang VARCHAR(10)
)
BEGIN
...
-- 8. 서비스 코드 전체 삭제
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_delete_all//

CREATE PROCEDURE sp_svccode_delete_all(
    IN p_sys_id VARCHAR(20)
)
BEGIN
    DELETE FROM SYS_CODE
...
-- 9. 서비스 코드 이력 등록
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_insert_hist//

CREATE PROCEDURE sp_svccode_insert_hist(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_desc VARCHAR(500),
...
-- ============================================================
-- 10. 서비스 코드 기간 목록 조회 (페이징)
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_term//

CREATE PROCEDURE sp_svccode_search_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_desc VARCHAR(200),
    ...
    SELECT * FROM (
        SELECT X.* FROM (
            ...
            FROM (
                ...
                WHERE SYS_ID = p_sys_id
                  AND (p_code_grup IS NOT NULL AND p_code_grup != '' AND CODE_GRUP = 'SERVICE_M' OR CODE_GRUP IS NULL)
                  AND CODE_GRUP != '0'
                  AND USE_FLAG = 'Y'
...
-- 11. 서비스 코드 기간 목록 카운트
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_term_count//

CREATE PROCEDURE sp_svccode_search_term_count(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_desc VARCHAR(200),
...
-- 12. 서비스 코드 기간 단건 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_select_term//

CREATE PROCEDURE sp_svccode_select_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_gs_lang VARCHAR(10)
)
...
-- 13. 서비스 코드 기간 등록
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_insert_term//

CREATE PROCEDURE sp_svccode_insert_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_desc VARCHAR(500),
...
BEGIN
    ...
    INSERT INTO SYS_CODE_TERM (
        SYS_ID, CODE_CD, CODE_GRUP, CODE_NAME, CODE_NAME_EN,
        CODE_DESC, CODE_DATE, SORT_SEQ, USE_FLAG,
        EXT_CHR01, EXT_CHR02, EXT_CHR03, EXT_CHR04, EXT_CHR05,
        EXT_CHR06, EXT_CHR07, EXT_CHR08, EXT_CHR09, EXT_CHR10,
        EXT_NUM01, EXT_NUM02, EXT_NUM03, EXT_NUM04, EXT_NUM05,
        EXT_TEXT, REGI_ID, REGI_DATE, CHNG_ID, CHNG_DATE
        ...
        p_sys_id, p_code_cd, v_code_grup, p_code_cd, p_code_cd,
        p_code_desc, p_code_date, p_sort_seq, IFNULL(p_use_flag, 'Y'),
        p_ext_chr01, p_ext_chr02, p_ext_chr03, p_ext_chr04, p_ext_chr05,
        p_ext_chr06, p_ext_chr07, p_ext_chr08, p_ext_chr09, p_ext_chr10,
        p_ext_num01, p_ext_num02, p_ext_num03, p_ext_num04, p_ext_num05,
        p_ext_text, p_gs_user_id, NOW(), p_gs_user_id, NOW()
...
-- 14. 서비스 코드 기간 삭제
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_delete_term//

CREATE PROCEDURE sp_svccode_delete_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_date VARCHAR(10)
)
...
-- ============================================================
-- 15. AS 코드 조회 (searchCodeAs)
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_code_as//

CREATE PROCEDURE sp_svccode_search_code_as(
    IN p_sys_id VARCHAR(20),
...
-- 16. 고객 제품 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_cust_prod//

CREATE PROCEDURE sp_svccode_search_cust_prod()
BEGIN
...
-- 17. 전체 제품 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_all_prod//

CREATE PROCEDURE sp_svccode_search_all_prod()
BEGIN
...
-- 18. 외주 고객 창고 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_svccode_search_outs_cust_strg//

CREATE PROCEDURE sp_svccode_search_outs_cust_strg()
BEGIN
