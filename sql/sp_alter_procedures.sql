-- ============================================================
-- Alter.xml (게시판 및 대상자 관리) -> 프로시저 전환 스크립트
-- 생성일: 2026-01-14
-- ============================================================
...
-- [sp_alter_search] 게시글 목록 조회 (페이징 및 동적 검색)
DROP PROCEDURE IF EXISTS sp_alter_search//
CREATE PROCEDURE sp_alter_search(
...
BEGIN
    ...
    SET @sql = "
        SELECT * FROM (
            SELECT X.* FROM (
                ...
                FROM (
                    SELECT 
                        A.SYS_ID AS sysId, A.BORD_NO AS bordNo, A.BORD_GRUP AS bordGrup, A.BORD_TITLE AS bordTitle,
                        A.BORD_TYPE AS bordType, A.BORD_BGN AS bordBgn, A.BORD_END AS bordEnd, A.READ_CNT AS readCnt,
                        A.BORD_SEQ AS bordSeq, A.BORD_PNO AS bordPno, A.OPEN_TYPE AS openType, A.USE_FLAG AS useFlag,
                        A.REGI_ID AS regiId, DATE_FORMAT(A.REGI_DATE,'%Y-%m-%d %H:%i:%s') AS regiDate,
                        A.CHNG_ID AS chngId, DATE_FORMAT(A.CHNG_DATE,'%Y-%m-%d %H:%i:%s') AS chngDate,
                        ...
                        A.BORD_NO AS id, A.BORD_PNO AS parentId,
                        CASE WHEN (SELECT COUNT(*) FROM SYS_BORD WHERE SYS_ID = A.SYS_ID AND BORD_PNO = A.BORD_NO AND BORD_GRUP = A.BORD_GRUP) = 0 THEN 'open' ELSE 'closed' END AS state,
                        (SELECT COUNT(Atch_No) FROM SYS_FILE WHERE SYS_ID = A.SYS_ID AND ATCH_NO = A.BORD_NO AND ATCH_GRUP = A.BORD_GRUP) AS fileCnt
                    FROM SYS_BORD A
                    WHERE A.SYS_ID = ? AND A.BORD_GRUP = ?
    ";

    IF p_use_flag != '' THEN SET @sql = CONCAT(@sql, " AND A.USE_FLAG = '", p_use_flag, "'"); END IF;
    IF p_regi_id != '' THEN SET @sql = CONCAT(@sql, " AND A.REGI_ID = '", p_regi_id, "'"); END IF;
    
    IF p_search_key = 'S01' THEN SET @sql = CONCAT(@sql, " AND A.BORD_TITLE LIKE '%", p_search_text, "%'");
    ELSEIF p_search_key = 'S02' THEN SET @sql = CONCAT(@sql, " AND A.BORD_TEXT LIKE '%", p_search_text, "%'");
    ELSEIF p_search_key = 'S03' THEN SET @sql = CONCAT(@sql, " AND EXISTS (SELECT 1 FROM SYS_USER WHERE SYS_ID = A.SYS_ID AND USER_ID = A.REGI_ID AND USER_NAME LIKE '%", p_search_text, "%')");
    END IF;

    IF p_bord_pno != '' THEN SET @sql = CONCAT(@sql, " AND A.BORD_PNO = '", p_bord_pno, "'");
    ELSE SET @sql = CONCAT(@sql, " AND A.BORD_PNO IS NULL"); END IF;
    ...
    IF p_today_yn = 'Y' THEN SET @sql = CONCAT(@sql, " AND DATE_FORMAT(NOW(),'%Y-%m-%d') BETWEEN A.BORD_BGN AND A.BORD_END"); END IF;

    SET @sql = CONCAT(@sql, " ORDER BY A.BORD_SEQ ASC, A.REGI_DATE DESC ) Z1, (SELECT @rownum:=0 ) Z2 ) X ");
    IF p_start IS NOT NULL AND p_end IS NOT NULL THEN SET @sql = CONCAT(@sql, " WHERE RNUM < ", p_end, " ) X WHERE RNUM >= ", p_start);
    ELSE SET @sql = CONCAT(@sql, " ) X "); END IF;

    PREPARE stmt FROM @sql;
    SET @p1 = p_sys_id; SET @p2 = p_bord_grup;
    EXECUTE stmt USING @p1, @p2;
    DEALLOCATE PREPARE stmt;
END//

-- [sp_alter_search_count] 게시글 카운트
DROP PROCEDURE IF EXISTS sp_alter_search_count//
CREATE PROCEDURE sp_alter_search_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_regi_id VARCHAR(50),
...
BEGIN
    SELECT COUNT(1) AS cnt
     ...
     WHERE A.SYS_ID = p_sys_id
       ...
       AND (CASE WHEN p_search_key = 'S01' AND p_search_text IS NOT NULL AND p_search_text != '' THEN A.BORD_TITLE LIKE CONCAT('%', p_search_text, '%')
...
-- [sp_alter_select] 게시글 상세 조회
DROP PROCEDURE IF EXISTS sp_alter_select//
CREATE PROCEDURE sp_alter_select(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20)
)
BEGIN
    SELECT
        A.SYS_ID        AS sysId,
    ...
    FROM SYS_BORD A
    WHERE A.SYS_ID = p_sys_id AND A.BORD_NO = p_bord_no AND A.BORD_GRUP = p_bord_grup;
END//

-- [sp_alter_insert] 게시글 등록
DROP PROCEDURE IF EXISTS sp_alter_insert//
CREATE PROCEDURE sp_alter_insert(
    IN p_sys_id VARCHAR(20), IN p_bord_grup VARCHAR(20), IN p_bord_title VARCHAR(200),
    IN p_bord_type VARCHAR(20), IN p_bord_bgn VARCHAR(20), IN p_bord_end VARCHAR(20),
    IN p_bord_pno VARCHAR(20), IN p_open_type VARCHAR(10), IN p_bord_text LONGBLOB, IN p_gs_user_id VARCHAR(50)
)
BEGIN
    ...
    INSERT INTO SYS_BORD (
        ...
        p_sys_id, v_new_no, p_bord_grup, p_bord_title, 0, 999999999, 'Y', p_gs_user_id, NOW(), p_gs_user_id, NOW(),
        p_bord_type, p_bord_bgn, p_bord_end, p_bord_pno, p_open_type, p_bord_text
    );
    SELECT v_new_no AS bordNo;
END//

-- [sp_alter_update] 게시글 전체 수정
DROP PROCEDURE IF EXISTS sp_alter_update//
CREATE PROCEDURE sp_alter_update(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20),
    IN p_bord_title VARCHAR(200), IN p_bord_text LONGBLOB, IN p_bord_type VARCHAR(20),
    IN p_bord_bgn VARCHAR(20), IN p_bord_end VARCHAR(20), IN p_read_cnt INT,
    IN p_bord_seq INT, IN p_use_flag VARCHAR(1), IN p_open_type VARCHAR(10), IN p_gs_user_id VARCHAR(50)
)
BEGIN
    UPDATE SYS_BORD
    SET CHNG_ID = p_gs_user_id, CHNG_DATE = NOW(),
...
-- [sp_alter_delete] 게시글 물리 삭제
DROP PROCEDURE IF EXISTS sp_alter_delete//
CREATE PROCEDURE sp_alter_delete(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20)
)
BEGIN
...
-- [sp_alter_update_read_cnt] 조회수 증가
DROP PROCEDURE IF EXISTS sp_alter_update_read_cnt//
CREATE PROCEDURE sp_alter_update_read_cnt(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20)
)
BEGIN
...
-- [sp_alter_update_disable] 게시글 사용안함 처리
DROP PROCEDURE IF EXISTS sp_alter_update_disable//
CREATE PROCEDURE sp_alter_update_disable(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20), IN p_gs_user_id VARCHAR(50)
)
BEGIN
...
-- [sp_alter_search_target] 대상자 목록 조회
DROP PROCEDURE IF EXISTS sp_alter_search_target//
CREATE PROCEDURE sp_alter_search_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    ...
    SELECT * FROM (
        SELECT X.* FROM (
            SELECT Z1.*, @rownum := @rownum + 1 AS RNUM FROM (
                ...
                ORDER BY B.BORD_SEQ ASC, A.REGI_DATE DESC
            ) Z1, (SELECT @rownum:=0 ) Z2
        ) X WHERE RNUM < p_end
    ) X WHERE RNUM >= p_start;
END//

-- [sp_alter_search_target_count] 대상자 목록 카운트 (페이징용)
DROP PROCEDURE IF EXISTS sp_alter_search_target_count//

CREATE PROCEDURE sp_alter_search_target_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_bord_no VARCHAR(20),
    ...
    IN p_vndr_name VARCHAR(100),
    IN p_save_idx INT,
    IN p_page_type VARCHAR(1),   -- 'R': 받은편지함, 'S': 보낸편지함
    IN p_gs_user_id VARCHAR(50)
)
BEGIN
    SELECT COUNT(1) AS cnt
    FROM SYS_BORD_TGT A
    INNER JOIN SYS_BORD B 
       ON B.SYS_ID    = A.SYS_ID 
      AND B.BORD_NO   = A.BORD_NO 
      AND B.BORD_GRUP = A.BORD_GRUP
    WHERE A.SYS_ID = p_sys_id 
      AND A.BORD_GRUP = p_bord_grup
      -- 동적 조건 처리 (XML의 whereClause-target 대응)
      AND (CASE WHEN p_use_flag IS NOT NULL AND p_use_flag != '' THEN A.USE_FLAG = p_use_flag ELSE 1=1 END)
      AND (CASE WHEN p_bord_no IS NOT NULL AND p_bord_no != '' THEN A.BORD_NO = p_bord_no ELSE 1=1 END)
      AND (CASE WHEN p_vndr_code IS NOT NULL AND p_vndr_code != '' THEN A.VNDR_CODE = p_vndr_code ELSE 1=1 END)
      AND (CASE WHEN p_vndr_name IS NOT NULL AND p_vndr_name != '' THEN A.VNDR_NAME = p_vndr_name ELSE 1=1 END)
      ...
      -- 수신/발신 구분
      AND (CASE WHEN p_page_type = 'R' THEN A.TGT_USER_ID = p_gs_user_id
                WHEN p_page_type = 'S' THEN A.REGI_ID = p_gs_user_id
                ELSE 1=1 END);
END//

-- [sp_alter_select_target] 대상자 단건 조회
DROP PROCEDURE IF EXISTS sp_alter_select_target//
CREATE PROCEDURE sp_alter_select_target(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20), IN p_tgt_user_id VARCHAR(50), IN p_use_flag VARCHAR(1)
)
BEGIN
    SELECT 
        A.*, B.BORD_TEXT AS bordText,
        (SELECT USER_NAME FROM SYS_USER WHERE SYS_ID = A.SYS_ID AND USER_ID = A.TGT_USER_ID) AS tgtName
    FROM SYS_BORD_TGT A
    INNER JOIN SYS_BORD B ON B.SYS_ID = A.SYS_ID AND B.BORD_NO = A.BORD_NO AND B.BORD_GRUP = A.BORD_GRUP
    WHERE A.SYS_ID = p_sys_id AND A.BORD_NO = p_bord_no AND A.BORD_GRUP = p_bord_grup AND A.TGT_USER_ID = p_tgt_user_id
    AND (CASE WHEN p_use_flag IS NOT NULL THEN A.USE_FLAG = p_use_flag ELSE 1=1 END);
END//

-- [sp_alter_insert_target] 대상자 등록
DROP PROCEDURE IF EXISTS sp_alter_insert_target//
CREATE PROCEDURE sp_alter_insert_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_targets TEXT,
...
BEGIN
    ...
    SELECT p_sys_id, p_bord_no, p_bord_grup, A.USER_ID, A.DEPT_CODE,
           ...
           'Y', p_gs_user_id, NOW(), p_gs_user_id, NOW(), (CASE WHEN p_read_yn = 'Y' THEN NOW() ELSE NULL END)
    FROM SYS_USER A
    WHERE A.SYS_ID = p_sys_id
    AND ( (p_targets IS NULL AND A.USER_ID = p_gs_user_id) OR (p_targets IS NOT NULL AND FIND_IN_SET(A.USER_ID, p_targets)) );
END//

-- [sp_alter_update_target] 대상자 정보 수정
DROP PROCEDURE IF EXISTS sp_alter_update_target//
CREATE PROCEDURE sp_alter_update_target(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50), IN p_vndr_code VARCHAR(20), IN p_vndr_name VARCHAR(100), IN p_save_idx INT, IN p_use_flag VARCHAR(1), IN p_gs_user_id VARCHAR(50)
)
BEGIN
    UPDATE SYS_BORD_TGT
    SET CHNG_ID = p_gs_user_id, CHNG_DATE = NOW(),
...
-- [sp_alter_update_target_disable] 대상자 비활성화 처리
DROP PROCEDURE IF EXISTS sp_alter_update_target_disable//
CREATE PROCEDURE sp_alter_update_target_disable(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
...
-- [sp_alter_delete_target_all] 대상자 일괄 삭제
DROP PROCEDURE IF EXISTS sp_alter_delete_target_all//
CREATE PROCEDURE sp_alter_delete_target_all(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20)
)
BEGIN
...
-- [sp_alter_update_target_read] 대상자 읽음 처리
DROP PROCEDURE IF EXISTS sp_alter_update_target_read//
CREATE PROCEDURE sp_alter_update_target_read(
    IN p_sys_id VARCHAR(20), IN p_bord_no VARCHAR(20), IN p_bord_grup VARCHAR(20), IN p_tgt_user_id VARCHAR(50)
)
BEGIN
