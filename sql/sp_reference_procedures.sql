...
-- ============================================================
-- 1. 게시글 관리 (SYS_BORD) - Reference용
-- ============================================================

-- [sp_reference_search] 게시글 목록 조회 (페이징 및 동적 검색)
DROP PROCEDURE IF EXISTS sp_reference_search//
CREATE PROCEDURE sp_reference_search(
...
-- [sp_reference_search_count] 게시글 카운트
DROP PROCEDURE IF EXISTS sp_reference_search_count//
CREATE PROCEDURE sp_reference_search_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_regi_id VARCHAR(50),
...
-- [sp_reference_select] 게시글 상세 조회
DROP PROCEDURE IF EXISTS sp_reference_select//
CREATE PROCEDURE sp_reference_select(
    IN p_sys_id VARCHAR(20),
...
-- [sp_reference_insert] 게시글 등록 (PK 자동 생성 포함)
DROP PROCEDURE IF EXISTS sp_reference_insert//
CREATE PROCEDURE sp_reference_insert(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_title VARCHAR(200),
    IN p_bord_type VARCHAR(20),
...
BEGIN
    ...
    INTO p_bord_no
    FROM SYS_BORD
    WHERE SYS_ID = p_sys_id AND BORD_GRUP = p_bord_grup;
    ...
    INSERT INTO SYS_BORD (
        ...
        p_sys_id, p_bord_no, p_bord_grup, p_bord_title, 0, 999999999, 'Y',
...
-- [sp_reference_update] 게시글 수정
DROP PROCEDURE IF EXISTS sp_reference_update//
CREATE PROCEDURE sp_reference_update(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_bord_title VARCHAR(200),
...
-- [sp_reference_delete] 게시글 물리 삭제
DROP PROCEDURE IF EXISTS sp_reference_delete//
CREATE PROCEDURE sp_reference_delete(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_reference_update_read_cnt] 조회수 증가
DROP PROCEDURE IF EXISTS sp_reference_update_read_cnt//
CREATE PROCEDURE sp_reference_update_read_cnt(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_reference_update_disable] 게시글 사용안함 처리
DROP PROCEDURE IF EXISTS sp_reference_update_disable//
CREATE PROCEDURE sp_reference_update_disable(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_gs_user_id VARCHAR(50)
...
-- ============================================================
-- 2. 대상자 관리 (SYS_BORD_TGT) - Reference용
-- ============================================================

-- [sp_reference_search_target] 대상자 목록 조회
DROP PROCEDURE IF EXISTS sp_reference_search_target//
CREATE PROCEDURE sp_reference_search_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_bord_no VARCHAR(20),
...
-- [sp_reference_search_target_count] 대상자 목록 카운트
DROP PROCEDURE IF EXISTS sp_reference_search_target_count//
CREATE PROCEDURE sp_reference_search_target_count(
    IN p_sys_id VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_use_flag VARCHAR(1),
    IN p_bord_no VARCHAR(20),
...
-- [sp_reference_select_target] 대상자 단건 조회
DROP PROCEDURE IF EXISTS sp_reference_select_target//
CREATE PROCEDURE sp_reference_select_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
...
-- [sp_reference_insert_target] 대상자 등록 (targets 리스트 -> 콤마 구분 문자열)
DROP PROCEDURE IF EXISTS sp_reference_insert_target//
CREATE PROCEDURE sp_reference_insert_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_targets TEXT,
...
BEGIN
    INSERT INTO SYS_BORD_TGT (
        SYS_ID, BORD_NO, BORD_GRUP, TGT_USER_ID, VNDR_CODE, VNDR_NAME,
        USE_FLAG, REGI_ID, REGI_DATE, CHNG_ID, CHNG_DATE, READ_DATE
    )
    SELECT
        p_sys_id,
        p_bord_no,
    ...
    WHERE A.SYS_ID = p_sys_id
      AND (
          (p_targets IS NULL AND A.USER_ID = p_gs_user_id)
          OR (p_targets IS NOT NULL AND FIND_IN_SET(A.USER_ID, p_targets))
      );
END//

-- [sp_reference_update_target] 대상자 정보 수정
DROP PROCEDURE IF EXISTS sp_reference_update_target//
CREATE PROCEDURE sp_reference_update_target(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50),
...
-- [sp_reference_update_target_disable] 대상자 비활성화 처리
DROP PROCEDURE IF EXISTS sp_reference_update_target_disable//
CREATE PROCEDURE sp_reference_update_target_disable(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50),
...
-- [sp_reference_delete_target_all] 대상자 일괄 삭제
DROP PROCEDURE IF EXISTS sp_reference_delete_target_all//
CREATE PROCEDURE sp_reference_delete_target_all(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20)
)
...
-- [sp_reference_update_target_read] 대상자 읽음 처리
DROP PROCEDURE IF EXISTS sp_reference_update_target_read//
CREATE PROCEDURE sp_reference_update_target_read(
    IN p_sys_id VARCHAR(20),
    IN p_bord_no VARCHAR(20),
    IN p_bord_grup VARCHAR(20),
    IN p_tgt_user_id VARCHAR(50)
