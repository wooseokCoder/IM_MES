-- ============================================================================
-- 파일명: sp_imes_tsap_stock_cvnd_query_procedures.sql
-- 설명: 재고정보 조회 - 고객 특별재고 (TSAP_STOCK_CVND 테이블)
-- 원본: ProActive TSAP_STOCK_CVND_QUERY.cs
-- 작성일: 2026-03-03
-- ============================================================================

-- ============================================================================
-- sp_imes_tsap_stock_cvnd_query1
-- 용도: MAT02A 고객 특별재고 조회
-- 원본: TSAP_STOCK_CVND_QUERY.TSAP_STOCK_CVND_QUERY1
--
-- AS-IS SQL:
--   SELECT S.PART_CODE, S.PART_NAME, S.PLANTS, S.STK_LOC,
--          S.QTY, S.QINS_FLAG, S.PART_UNIT,
--          S.BATCH_NO, S.STOCK_EMP, S.CVND_CODE, S.CVND_NAME
--   FROM TSAP_STOCK_CVND S
...
--   - hinsFlag: CVND 타입에는 없음 → NULL
--   - IS_FILE: IF_PLM_FILE_INFO LEFT JOIN (도면 여부 확인)
-- ============================================================================
DELIMITER //

DROP PROCEDURE IF EXISTS sp_imes_tsap_stock_cvnd_query1//

CREATE PROCEDURE sp_imes_tsap_stock_cvnd_query1(
    IN p_plt_code   VARCHAR(10),   -- 세션 공장코드 (현재 SQL 미사용)
    IN p_part_like  VARCHAR(100)   -- 자재번호/명 LIKE 검색어
)
BEGIN
    SELECT
         '고객 특별재고'                                        AS stockType
        ...
        ,NULL                                                   AS orderNo
        ,NULL                                                   AS orderLine
        ,S.CVND_CODE                                            AS cvndCode
        ,S.CVND_NAME                                            AS cvndName
        ,NULL                                                   AS mvndCode
        ,NULL                                                   AS mvndName
