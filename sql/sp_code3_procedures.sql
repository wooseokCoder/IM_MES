...
-- 1. 코드 목록 조회 (페이징)
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search//

CREATE PROCEDURE sp_code3_search(
...
BEGIN
    SET @rownum := 0;
    SET @sort_order := IFNULL(p_sort_str, 'SORT_SEQ');
            ...
            SELECT X.* FROM (
                ...
                FROM (
                    ...
                    WHERE SYS_ID = ''', p_sys_id, '''
                      AND (
                          (''', IFNULL(p_code_grup, ''), ''' IS NOT NULL AND ''', IFNULL(p_code_grup, ''), ''' != '''' AND
                           CODE_GRUP = CASE WHEN ''', IFNULL(p_code_grup, ''), ''' = ''ALL'' THEN CODE_GRUP ELSE ''', IFNULL(p_code_grup, ''), ''' END
                           AND (CODE_GRUP != ''0'' OR CODE_CD != ''0''))
                          OR
                          ((''', IFNULL(p_code_grup, ''), ''' IS NULL OR ''', IFNULL(p_code_grup, ''), ''' = '''') AND CODE_GRUP = ''0'' AND CODE_CD != ''0'')
                      )');

    IF p_code_cd IS NOT NULL AND p_code_cd != '' THEN
        SET @sql := CONCAT(@sql, ' AND CODE_CD LIKE CONCAT(''%'', ''', p_code_cd, ''', ''%'')');
    END IF;

    IF p_code_desc IS NOT NULL AND p_code_desc != '' THEN
        SET @sql := CONCAT(@sql, ' AND (CODE_NAME LIKE CONCAT(''%'', ''', p_code_desc, ''', ''%'') OR CODE_DESC LIKE CONCAT(''%'', ''', p_code_desc, ''', ''%'') OR EXT_TEXT LIKE CONCAT(''%'', ''', p_code_desc, ''', ''%''))');
    END IF;
    ...
    IF p_use_flag IS NOT NULL AND p_use_flag != '' THEN
        SET @sql := CONCAT(@sql, ' AND USE_FLAG = CASE WHEN ''', p_use_flag, ''' = ''ALL'' THEN USE_FLAG ELSE ''', p_use_flag, ''' END');
    END IF;
    ...
    SET @sql := CONCAT(@sql, ' ORDER BY ', @sort_order, '
            ...
            WHERE (', IFNULL(p_end, 0), ' = 0 OR RNUM < ', IFNULL(p_end, 0), ')
        ) X
        WHERE (', IFNULL(p_start, 0), ' = 0 OR RNUM >= ', IFNULL(p_start, 0), ')');
...
-- 2. 코드 목록 카운트
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_count//

CREATE PROCEDURE sp_code3_search_count(
    IN p_sys_id VARCHAR(20),
    IN p_code_grup VARCHAR(50),
    IN p_code_cd VARCHAR(50),
    IN p_code_desc VARCHAR(200),
...
-- 3. 코드 단건 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_select//

CREATE PROCEDURE sp_code3_select(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_gs_lang VARCHAR(10)
...
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_insert//

CREATE PROCEDURE sp_code3_insert(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
...
-- 5. 코드 수정
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_update//

CREATE PROCEDURE sp_code3_update(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_name_kr VARCHAR(200),
...
BEGIN
    ...
    SET CHNG_ID = p_gs_user_id,
        CHNG_DATE = NOW(),
        CODE_NAME = p_code_name_kr,
        CODE_NAME_EN = p_code_name_en,
        CODE_DESC = p_code_desc,
        SORT_SEQ = p_sort_seq,
        USE_FLAG = p_use_flag,
        EXT_CHR01 = p_ext_chr01,
        EXT_CHR02 = p_ext_chr02,
        EXT_CHR03 = p_ext_chr03,
        EXT_CHR04 = p_ext_chr04,
        EXT_CHR05 = p_ext_chr05,
        EXT_CHR06 = p_ext_chr06,
        EXT_CHR07 = p_ext_chr07,
        EXT_CHR08 = p_ext_chr08,
        EXT_CHR09 = p_ext_chr09,
        EXT_CHR10 = p_ext_chr10,
        EXT_TEXT = 'Y'
    WHERE SYS_ID = p_sys_id
      AND CODE_CD = p_code_cd
      AND CODE_GRUP = p_code_grup;
...
-- 6. 코드 삭제
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_delete//

CREATE PROCEDURE sp_code3_delete(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50)
)
...
-- 7. 전체 코드 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_all//

CREATE PROCEDURE sp_code3_search_all(
    IN p_sys_id VARCHAR(20),
    IN p_gs_lang VARCHAR(10)
)
BEGIN
...
-- 8. 코드 그룹 전체 삭제
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_delete_all//

CREATE PROCEDURE sp_code3_delete_all(
    IN p_sys_id VARCHAR(20),
    IN p_code_grup VARCHAR(50)
)
BEGIN
...
-- NULLIF 패턴: 빈문자열이면 NULL로 변환
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_insert_hist//

CREATE PROCEDURE sp_code3_insert_hist(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_name_kr VARCHAR(200),
...
-- 10. 코드 기간 목록 조회 (페이징)
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_term//

CREATE PROCEDURE sp_code3_search_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_grup VARCHAR(50),
    IN p_code_cd VARCHAR(50),
    IN p_code_desc VARCHAR(200),
        ...
        SELECT * FROM (
            SELECT X.* FROM (
                ...
                FROM (
                    ...
                    WHERE SYS_ID = ''', p_sys_id, '''
                      AND (''', IFNULL(p_code_grup, ''), ''' IS NULL OR ''', IFNULL(p_code_grup, ''), ''' = '''' OR CODE_GRUP = ''', IFNULL(p_code_grup, ''), ''')
                      AND CODE_GRUP != ''0''
                      AND USE_FLAG = ''Y''');
...
-- 11. 코드 기간 목록 카운트
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_term_count//

CREATE PROCEDURE sp_code3_search_term_count(
    IN p_sys_id VARCHAR(20),
    IN p_code_grup VARCHAR(50),
    IN p_code_cd VARCHAR(50),
    IN p_code_desc VARCHAR(200),
...
-- 12. 코드 기간 단건 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_select_term//

CREATE PROCEDURE sp_code3_select_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_gs_lang VARCHAR(10)
...
-- 13. 코드 기간 등록
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_insert_term//

CREATE PROCEDURE sp_code3_insert_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_name_kr VARCHAR(200),
...
-- 14. 코드 기간 삭제
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_delete_term//

CREATE PROCEDURE sp_code3_delete_term(
    IN p_sys_id VARCHAR(20),
    IN p_code_cd VARCHAR(50),
    IN p_code_grup VARCHAR(50),
    IN p_code_date VARCHAR(20)
...
-- 15. AS 코드 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_code_as//

CREATE PROCEDURE sp_code3_search_code_as(
    IN p_sys_id VARCHAR(20),
...
-- 16. 고객 제품 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_cust_prod//

CREATE PROCEDURE sp_code3_search_cust_prod()
BEGIN
...
-- 17. 전체 제품 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_all_prod//

CREATE PROCEDURE sp_code3_search_all_prod()
BEGIN
...
-- 18. 외주 고객 창고 조회
-- ============================================================
DROP PROCEDURE IF EXISTS sp_code3_search_outs_cust_strg//

CREATE PROCEDURE sp_code3_search_outs_cust_strg()
BEGIN
